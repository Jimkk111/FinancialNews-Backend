...
1. Extract topic from arguments. If no topic provided, ask in plain text: "What topic do you want to learn today?"
...
- "Here's broken code using [concept]. Fix it."
- "Refactor this code to use [concept] instead of [old approach]."

**For conceptual topics**:
- "Explain [concept] to a beginner. Write it out."
- "Design a system that uses [concept] for [use case]."
- "Create a comparison table: [concept] vs [related concept]."

**Practice evaluation**:
- Correct execution → mark `mastered`, set review interval to 1 day
- Minor issues → give targeted feedback, let them fix
- Major issues → cycle back to tutoring, NOT a mastery failure (they passed the check)

### Step 4: Session End

When the learner indicates they're done, or all concepts are mastered:

1. **Generate summary** -> `summary.html`
   - See [references/html-templates.md](references/html-templates.md) for template
   - Stats: concepts mastered, questions answered, time spent
   - Key insights: what the learner struggled with, what clicked
   - Next steps: recommended follow-up topics

2. **Update learner profile** -> `sigma/learner-profile.md`
   - Learning style observations (e.g., "prefers concrete examples before abstraction")
   - Recurring misconception patterns
   - Strong and weak areas across topics
   - Recommended focus areas for future sessions

3. **Say goodbye** with encouragement and a note about spaced repetition:
   > "Great session! You mastered X concepts. I've saved your progress — you can continue anytime with `/sigma {topic} --resume`. See you next time!"
...
| File | Purpose |
|------|---------|
| `pedagogy.md` | Bloom's 2-Sigma, Socratic method, mastery scoring, misconception handling |
| `html-templates.md` | HTML templates for roadmap, summary, and visual explanations |
| `excalidraw.md` | Excalidraw diagram generation guide |
